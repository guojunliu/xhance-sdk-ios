...
- (instancetype)init {
    ...
    if (self) {
        ...
        _adRealmFilePath = XhanceSDKSessionAdRealmFilePath;
    }
    return self;
}
...
- (void)writeAdRealmSession:(XhanceSessionModel *)sessionModel {
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_BACKGROUND, 0), ^{
        [self writeSession:sessionModel path:_adRealmFilePath];
    });
}

- (void)removeAdRealmSession:(XhanceSessionModel *)sessionModel {
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_BACKGROUND, 0), ^{
        [self removeSession:sessionModel path:_adRealmFilePath];
    });
}

- (NSArray <XhanceSessionModel *> *)getAdRealmSessions {
    return [self getSessionsForPath:_adRealmFilePath];
}
